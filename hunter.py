...
def scan_hunter(hoy):
    ...
    viewstate_LogIn = extraer_texto(respLogin.text, 'id="__VIEWSTATE" value="', '"')
    ...
    payload_LogInV3 = (
        ...
        + viewstategenerator_LogIn
        + "&__EVENTVALIDATION="
        + urllib.parse.quote(eventvalidation_LogIn, safe="")
        + "&txusuario="
        + USUARIO
        + "&txclave="
        + CLAVE
    ...
    respMain36 = requests.request("GET", HUN_URL_MAIN36, headers=headers_Main36)

    # print(respMain36.text)
    time_Main36 = extraer_texto(respMain36.text, 'id="hdwuid" value="', '"')
    ...
    time_EstadoFlota = extraer_texto(respEstadoFlota.text, 'id="hdwuid" value="', '"')
    ...
    payload_DescargarExcel = (
        ...
        + urllib.parse.quote(viewstate_EstadoFlota, safe="")
        + "&__VIEWSTATEGENERATOR="
        + viewstategenerator_EstadoFlota
        + "&__EVENTVALIDATION="
        + urllib.parse.quote(eventvalidation_EstadoFlota, safe="")
        + "&imgExcel.x=4&imgExcel.y=17&kmsResumen=%7B%26quot%3BcallbackState%26quot%3B%3A%26quot%3B%26quot%3B%7D&grdEstadoFlota="
        + urllib.parse.quote(
            INICIO_GRESTADOFLOTA + callbackstate_EstadoFlota + FIN_GRESTADOFLOTA,
            safe="",
        )
    ...
    nombreArchivo_Hunter_lambda = (
        "/tmp/Hunter_Excel_Python_" + fecha_nombrearchivo + ".xlsx"
    )

    respDescargaExcel = requests.request(
        "POST",
        HUN_URL_ESTADOFLOTA + time_Main36,
    ...
    df = df.rename(
        columns={
            df.columns[1]: "fecha_ultima_actualizacion",
            df.columns[4]: "direccion",
        }
    )
    # print(df.columns)
    # Contar caracteres de columa 'Ultimo reporte

    df["fecha_ultima_actualizacion"] = df["fecha_ultima_actualizacion"].astype(str)
    df["temp_fecha"] = df["fecha_ultima_actualizacion"].str.len()
    df["temp_fecha"] = df["temp_fecha"].astype(np.int64)
    # print(df)
    df["fecha"] = df.apply(
        lambda x: x["fecha_ultima_actualizacion"][0:10], axis=1
    )  # extrae la placa
    df["hora"] = df.apply(
        lambda x: x["fecha_ultima_actualizacion"][11 : x["temp_fecha"] - 1], axis=1
    )  # extrae la placa
