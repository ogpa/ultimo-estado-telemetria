...
def scan_geotab(hoy):
    ...
    for x in range(cant_dbs):
        print(dbs[x])
        api = mygeotab.API(
            username="bot-telemetria@mb-renting.com",
            password="FlotasMBRenting2k23$",
            database=dbs[x],
        )
        ...
        for s in dsi:
            ...
            lista_tallermolina.append(
                POLY_TALLER_MOLINA.contains(Point(s["latitude"], s["longitude"]))
            )
            ...
            multi_calls_odometro.append(
                [
                    ...
                    dict(
                        typeName="StatusData",
                        search={
                            "fromDate": ahora_geotab,
                            "toDate": ahora_geotab,
                            "deviceSearch": {"id": s["device"]["id"]},
        ...
        for r_o in r_multi_odometro:
            if len(r_o) > 0:
                ...
                odometro = r_o[0]["data"]
                lista_odometro.append(odometro / ODOMETRO_METROS_A_KILOMETROS)
