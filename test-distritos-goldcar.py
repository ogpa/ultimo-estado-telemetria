...
def identificar_distrito():
    df_goldcar = pd.read_csv("goldcar.csv")
    df = pd.read_csv("distritos_peru.csv", sep=";")
    # 1 Geo Shape
    # 3 NOMBDEP
    # 5 NOMBPROV
    # 7 NOMBDIST
    ...
    df_goldcar[["distrito", "provincia", "region"]] = df_goldcar.apply(lambda x: asignar_distrito(
        x["latitud"], x["longitud"], df, cantidad_distritos), axis='columns', result_type='expand')

    df_goldcar.to_csv("distritos_goldcar.csv", index=False)
