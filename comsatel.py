...
COM_URL_FOUND = (
    "http://clocatorplus.comsatel.com.pe/CL/faces/seguridad/login.xhtml;jsessionid="
)
...
CABECERA_BNI = "BNI_persistence="  # "BNI_persistence="
IDT_FOUND = "j_idt19"
COM_URL_PREREPORTE = (
    "http://clreportes.comsatel.com.pe/CLReporte/faces/index.xhtml;jsessionid="
)
...
def scan_comsatel(hoy):
    ...
    session_LogIn = extraer_texto(respLogin.headers["Set-Cookie"], "JSESSIONID=", ";")
    print(session_LogIn)
    ...
    payload_Found = (
        "frmLogin=frmLogin&usuario="
        + USUARIO
        + "&clave="
        + CLAVE
        + "&"
        + IDT_FOUND
        + "=Ingresar&javax.faces.ViewState="
    ...
    headers_Found = {
        ...
        + "; "
        + CABECERA_BNI
        + cookieBarracuda_LogIn,
        "Origin": COM_URL_BASE_CLOCATOR,
        "Referer": COM_URL_LOGIN,
        "Upgrade-Insecure-Requests": "1",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36",
    }
    # http://clocatorplus.comsatel.com.pe/CL/faces/seguridad/login.xhtml;jsessionid=0f2c98c440dff6f3c9710d05a696
    ...
    # Este payload es el de que aparece cuando se hace click en "Unidades sin reportar" y justo antes de que abra el popup de Reportes
    payload_prePopUpReporte = (
        "jjavax.faces.partial.ajax=true&javax.faces.source="
        + IDT_PREPOPUPREPORTE
        + "%3AfnOpenItemMenu&javax.faces.partial.execute=%40all&"
        + IDT_PREPOPUPREPORTE
        + "%3AfnOpenItemMenu="
        + IDT_PREPOPUPREPORTE
        + "%3AfnOpenItemMenu&pFuncionalidadId=437&pPadreId=8&pIrPagina=http%3A%2F%2Fclreportes.comsatel.com.pe%2FCLReporte%2F&"
        + IDT_PREPOPUPREPORTE
        + "="
        + IDT_PREPOPUPREPORTE
    ...
    respprePopUpReporte = requests.request(
        "POST",
        COM_URL_MAIN,
        headers=headers_prePopUpReporte,
        data=payload_prePopUpReporte,
    )
    ...
    cookie_prePopUpReporte = extraer_texto(
        respprePopUpReporte.headers["Set-Cookie"], "", "; "
    )
    ...
    payload_preReporte = (
        "javax.faces.partial.ajax=true&javax.faces.source=frmSesionLogin%3A"
        + IDT_PREREPORTE
        + "&javax.faces.partial.execute=%40all&frmSesionLogin%3A"
        + IDT_PREREPORTE
        + "=frmSesionLogin%3A"
        + IDT_PREREPORTE
        + "&frmSesionLogin=frmSesionLogin&javax.faces.ViewState="
    ...
    headers_preReporte = {
        ...
        + session_CLReporte
        + ";"
        + cookie_prePopUpReporte
        + "; "
        + CABECERA_BNI
        + cookieBarracuda_CLReporte,
        "Faces-Request": "partial/ajax",
    ...
    # Este figura como index.xhtml;jsessionid=0fc9b495cb9379053a6b05aef5af en la columna Name de Network
    resppreReporte = requests.request(
        "POST",
        COM_URL_PREREPORTE + session_CLReporte,
        headers=headers_preReporte,
        data=payload_preReporte,
    )
    # print("resppreReporte.headers")
    # print(resppreReporte.headers)
    ...
    respventanaReporte = requests.request(
        "GET",
        COM_URL_VEHICULOSINREPORTAR,
        headers=headers_ventanaReporte,
        data=payload_ventanaReporte,
    )
    ...
    respclickDetalle = requests.request(
        "POST",
        COM_URL_VEHICULOSINREPORTAR,
        headers=headers_clickDetalle,
        data=payload_clickDetalle,
    )
    ...
    respclickBuscar = requests.request(
        "POST",
        COM_URL_VEHICULOSINREPORTAR,
        headers=headers_clickBuscar,
        data=payload_clickBuscar,
    )
    # Quizás falta agregar al request anterior: allow_redirects=False,

    # print(respclickBuscar.text)
    # print(cookieBarracuda_CLReporte)
    # print(respclickBuscar.headers)
    viewstate_clickBuscar = extraer_texto(
        respclickBuscar.text,
        'id="j_id1:javax.faces.ViewState:0" value="',
        '" autocomplete="off"',
    )
    ...
    # Aquí sale el onclick="noSalirSistema();">Alarmas</a> que es cuando se descarga un Excel mal
    response_clickBuscar_2 = requests.request(
        "GET",
        COM_URL_VEHICULOSINREPORTAR,
        headers=headers_clickBuscar_2,
        data=payload_clickBuscar_2,
    )
    # print(response_clickBuscar_2.text)

    payload_PopUpExcel = (
        "javax.faces.partial.ajax=true&javax.faces.source=frmListar%3AbtnExportar&javax.faces.partial.execute=%40all&frmListar%3AbtnExportar=frmListar%3AbtnExportar&frmListar=frmListar&frmListar%3AchkDetalle_input=on&frmListar%3AdtVehiculosSinReportar_selection=&javax.faces.ViewState="
    ...
    respPopUpExcel = requests.request(
        "POST",
        COM_URL_VEHICULOSINREPORTAR,
        headers=headers_PopUpExcel,
        data=payload_PopUpExcel,
    )
    ...
    nombreArchivo_Comsatel_local = (
        "Comsatel_Excel_Python_" + fecha_nombrearchivo + ".xlsx"
    )
    nombreArchivo_Comsatel_lambda = (
        "/tmp/Comsatel_Excel_Python_" + fecha_nombrearchivo + ".xlsx"
    )
    ...
    # Manipulación
    POLY_TALLER_MOLINA = Polygon(
        [
            (-12.071496, -76.955457),
            (-12.071008, -76.954843),
            (-12.070704, -76.953837),
            (-12.072157, -76.953322),
            (-12.0726576, -76.954998),
        ]
    )
    ...
    df["Alias"] = df["Placa"]
    df["Placa"] = (
        df["Placa"].str[:3] + "-" + df["Placa"].str[3:]
    )  # Coloca guión a la placa
    df["Taller Molina"] = df.apply(
        lambda x: POLY_TALLER_MOLINA.contains(Point(x["Latitud"], x["Longitud"])),
        axis=1,
    )
    # df["Region"] = df.apply(lambda x: asignar_region(
    #     x["Latitud"], x["Longitud"]), axis=1)
    df["Fecha Ultima Localizacion"] = df["Fecha Ultima Localizacion"].astype(str)
    df["temp_fecha"] = df["Fecha Ultima Localizacion"].str.len()
    df["temp_fecha"] = df["temp_fecha"].astype(np.int64)
    # print(df)
    df["Fecha"] = df.apply(
        lambda x: x["Fecha Ultima Localizacion"][0:10], axis=1
    )  # extrae la placa
    df["Hora"] = df.apply(
        lambda x: x["Fecha Ultima Localizacion"][11 : x["temp_fecha"] - 1], axis=1
    )  # extrae la placa
